...
from parser.validation import TextValidator, ValidationResult
...
class VisionProcessor:
  ...
  def process_page(self, image_base64: str) -> str:
    ...
    try:
      ...
      self.logger.debug(
        f"Vision model response received, length: {len(response.content)}"
      )
...
class PDFToMarkdownPipeline:
  ...
  def __init__(self,
    ...
    if self.enable_validation:
      ...
      self.logger.info(
        f"Text validation enabled with threshold: {validation_threshold}"
      )
  ...
  def convert_pdf(self, pdf_path: str) -> ConversionResult:
    ...
    try:
      ...
      with fitz.open(str(pdf_path)) as doc:
        ...
        for page_num in range(doc.page_count):
          try:
            page = doc[page_num]
            self.logger.info(
              f"Processing page {page_num + 1}/{doc.page_count}"
            )
      ...
      v_res = None # Validation results
      if self.enable_validation and pages_markdown:
        self.logger.info("Running text validation...")
        v_res = self.validator.validate_document(str(pdf_path), pages_markdown)
        
        # Log validation summary
        passed_count = sum(1 for r in v_res if r.passed_threshold)
        avg_score = sum(r.validation_score for r in v_res) / len(v_res)
        self.logger.info(
          f"Validation complete: {passed_count}/{len(v_res)} pages passed "
      ...
      if v_res:
        passed_validation = sum(1 for r in v_res if r.passed_threshold)
        avg_v_score = sum(r.validation_score for r in v_res) / len(v_res)
        res_metadata.update({
          "validation": {
            "pages_passed": passed_validation,
            "total_pages": len(v_res),
            "pass_rate": passed_validation / len(v_res),
            "average_score": avg_v_score
          }
        })
  ...
  def save_results(self,
                   ...
                   output_dir: str = "./output") -> List[Path]:
