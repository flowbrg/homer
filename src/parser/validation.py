...
STOPWORDS = {'the', 'and', 'are', 'for', 'this', 'that', 'with', 'from',
      ...
      'find', 'long', 'down', 'day', 'did', 'get', 'come', 'made', 'part',
      'over'}
...
class TextValidator:
  ...
  def _calculate_sentence_overlap(self,
    ...
    for ext_sent in extracted_sentences:
      ...
      for llm_sent in llm_sentences:
        ...
        if len(ext_words & llm_words) / len(ext_words) > 0.6:
          # 60% word overlap
          matches += 1
          break
  ...
  def _find_missing_important_words(self,
                                    extracted_words: List[str],
                                    llm_words: List[str]) -> Set[str]:
  ...
  def _calculate_extra_content_ratio(self,
                                     extracted: str,
                                     llm_text: str) -> float:
  ...
  def _log_validation_result(self, result: ValidationResult):
    ...
    if result.missing_words:
      logger.debug(
        f"  Missing important words: {list(result.missing_words)[:5]}"
      )
    
    if result.extra_content_ratio > 0.5:
      logger.debug(
        f"  Extra content ratio: {result.extra_content_ratio:.3f}"
      )
  ...
  def validate_page(self,
                    page: fitz.Page,
                    llm_markdown: str) -> ValidationResult:
    ...
    sentence_overlap = self._calculate_sentence_overlap(extracted_text,
                                                        llm_text)
    semantic_sim = self._calculate_semantic_similarity(extracted_text,
                                                       llm_text)
    ...
    missing_words = self._find_missing_important_words(extracted_words,
                                                       llm_words)
    
    # Calculate extra content ratio
    extra_content = self._calculate_extra_content_ratio(extracted_text,
                                                        llm_text)
    
    # Calculate overall validation score
    validation_score = self._calculate_validation_score(word_overlap,
      char_overlap, sentence_overlap, semantic_sim)
  ...
  def validate_document(self, pdf_path: str, pages_markdown: List[str]) -> List[ValidationResult]:
    ...
    self.logger.info(
      f"Document validation: {passed_count}/{len(results)} pages passed "
